      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-20 text-center">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {naicsList.map((item, index) => (
            <motion.div
              key={index}
              className="flex flex-col space-y-4 bg-gray-900 p-6 border-2 border-green-400 rounded-lg shadow-lg transition-transform duration-300 hover:scale-105 hover:shadow-green-400"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: index * 0.2 }}
              whileHover={{ scale: 1.05 }}
              style={{ height: "220px" }}  // Ensures consistent height for all cards
            >
              <p className="whitespace-normal text-white text-lg md:text-xl">
