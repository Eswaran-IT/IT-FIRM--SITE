...
const ServiceCard = ({ animation, title, index, activeIndex, totalServices }) => {
  // Calculate the circular effect positioning
  ...
  return (
    <motion.div
      ...
      style={{
        transform: `scale(${scale}) rotateY(${rotateY}deg) translateZ(200px)`,
        zIndex: zIndex,
      }}
    >
...
const Services = () => {
  ...
  return (
    <section className="py-20 bg-black flex flex-col items-center relative">
      ...
      <button
        onClick={prevService}
        className="absolute left-4 text-white bg-green-400 rounded-full p-2 shadow-lg hover:bg-green-500"
      >
        {"<"}
      </button>
      <button
        onClick={nextService}
        className="absolute right-4 text-white bg-green-400 rounded-full p-2 shadow-lg hover:bg-green-500"
      >
        {">"}
      </button>
