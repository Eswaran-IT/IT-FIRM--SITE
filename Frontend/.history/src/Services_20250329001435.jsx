...
const getCardStyle = (index, activeIndex, length) => {
  ...
  let zIndex = 1;
  let heightClass = "h-60"; // Default height for side cards

  if (index === activeIndex) {
    transformStyle = "scale(1) rotateY(0deg)";
    zIndex = 3;
    heightClass = "h-80"; // Larger height for the center card
  ...
  return { transformStyle, zIndex, heightClass };
};

// Service Card Component
const ServiceCard = ({ animation, title, index, activeIndex, length }) => {
  const { transformStyle, zIndex, heightClass } = getCardStyle(index, activeIndex, length);

  return (
    <motion.div
      className={`absolute bg-gray-900 rounded-lg shadow-lg p-6 border border-green-400 transition-all duration-500 ${heightClass}`}
