...
const ServiceCard = ({ animation, title, index, activeIndex, totalServices }) => {
  ...
  const scale = index === activeIndex ? 1.2 : 0.9;  // Scale larger for focused card
...
const Services = () => {
  ...
  return (
    <section className="py-20 bg-black flex flex-col items-center relative">
      ...
      {/* Carousel Container */}
      <div className="relative w-full sm:w-3/4 md:w-1/2 lg:w-1/3 flex justify-center items-center h-96 sm:h-[500px] overflow-hidden mt-12">
        
        {/* Arrows Container (with margin on sides) */}
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 z-10 mx-8 md:mx-16">
          <button
            onClick={prevService}
        ...
        {/* Arrows Container (with margin on sides) */}
        <div className="absolute right-0 top-1/2 transform -translate-y-1/2 z-10 mx-8 md:mx-16">
          <button
            onClick={nextService}
            className="text-white bg-green-400 rounded-full p-4 shadow-lg hover:bg-green-500"
          >
