...
const ServiceCard = ({ animation, title, index, activeIndex, totalServices, backgroundImage }) => {
  ...
  const rotateY = offset * (angle * 0.4); // reduced rotation
  const scale = index === activeIndex ? 1.1 : 0.95;
  const zIndex = index === activeIndex ? 3 : 1;
  const cardSize = "w-80 h-72"; // wider and taller cards
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-black bg-opacity-70 backdrop-blur-sm rounded-lg"></div>
      <div className="relative flex flex-col items-center justify-center text-center rounded-lg p-6 
        ...
        <h3 className="text-lg font-bold text-white mt-4 relative z-10 px-2 text-center">
          {title}
        </h3>
      </div>
    </motion.div>
