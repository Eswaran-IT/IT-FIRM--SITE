...
const Analyzer = () => {
  ...
  const newVsReturning = [
    { name: "New Visitors", value: data?.userTypeStats?.newUsers || 0 },
    { name: "Returning Visitors", value: data?.userTypeStats?.returningUsers || 0 },
  ];

  const deviceBreakdown = data?.deviceStats || [];
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      {/* 4. Avg Time Spent */}
      <div className="bg-white p-6 rounded-xl shadow text-center mb-8">
        <h2 className="text-xl font-semibold mb-2 text-gray-700">⏱️ Avg Time Spent</h2>
        <p className="text-2xl font-bold text-indigo-600">{formatTime(data?.avgTimeSpent || 0)}</p>
      </div>

      {/* 5. New vs Returning Visitors */}
      <div className="bg-white p-6 rounded-xl shadow mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-700">🆕 New vs Returning Visitors</h2>
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              data={newVsReturning}
      ...
      <div className="bg-white p-6 rounded-xl shadow mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-700">📱 Device / OS / Browser</h2>
        <ResponsiveContainer width="100%" height={300}>
