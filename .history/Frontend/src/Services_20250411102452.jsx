...
const ServiceCard = ({ animation, title, index, activeIndex, totalServices, backgroundImage }) => {
  ...
  const cardSize = isActive ? "w-64 h-64" : "w-72 h-64"; // Wider side cards

  return (
    <motion.div
      className={`absolute ${cardSize} rounded-lg p-6 transition-all duration-500`}
      style={{
    ...
    >
      ...
      {/* Keep content facing forward even on rotated cards */}
      <div
        className="relative flex flex-col items-center justify-center text-center rounded-lg p-6 
...
const Services = () => {
  ...
  return (
    <section
      className="relative py-12 md:py-6 flex flex-col items-center justify-center min-h-screen"
      style={{
    ...
    >
      ...
      {/* Carousel */}
      <div className="relative w-full flex justify-center items-center h-72 sm:h-80 overflow-hidden mt-8 md:mt-16 z-10">
        {/* Left Arrow */}
        <button
          onClick={() => setActiveIndex((prevIndex) => (prevIndex - 1 + totalServices) % totalServices)}
          className="absolute left-[5%] sm:left-[10%] bg-white bg-opacity-20 hover:bg-opacity-40 text-white text-xl sm:text-2xl p-1 sm:p-2 rounded-full shadow-lg z-20"
        >
          &#8592;
        </button>
        ...
        <button
          ...
          className="absolute right-[5%] sm:right-[10%] bg-white bg-opacity-20 hover:bg-opacity-40 text-white text-xl sm:text-2xl p-1 sm:p-2 rounded-full shadow-lg z-20"
        >
          &#8594;
        </button>
      </div>
