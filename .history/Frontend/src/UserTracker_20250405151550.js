...
export const trackAction = (actionName) => {
  ...
  console.log(`🖱️ Action tracked: ${actionName}`);
};

// Send tracking data
export const sendTrackingData = () => {
  try {
    console.log("📡 Preparing to send tracking data...");
    ...
    if (!success) {
      console.warn("⚠️ Beacon failed, using fetch fallback...");
      ...
      fetch(url, {
        ...
        else console.warn("❌ Fetch fallback failed. Status:", res.status);
      }).catch(err => {
...
console.log("📲 sendTrackingData() is hooked to beforeunload event.");
