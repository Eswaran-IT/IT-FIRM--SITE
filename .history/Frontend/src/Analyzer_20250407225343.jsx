    ...
    <div className="h-[100vh] overflow-y-auto bg-black px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <ChartContainer title="⏰ Peak Hours">
          <ResponsiveContainer>
            <AreaChart data={data.peakHours || []}>
              ...
              <YAxis stroke="#ccc" />
              <Tooltip />
              <Area dataKey="count" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.3} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartContainer>
        ...
        <ChartContainer title="📱 Device Usage">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={data.devices || []} dataKey="count" nameKey="name" outerRadius={70}>
        ...
        <ChartContainer title="🌐 Browser Usage">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={data.browsers || []} dataKey="count" nameKey="name" outerRadius={70}>
                {(data.browsers || []).map((entry, i) => (
                  <Cell key={i} fill={COLORS[i % COLORS.length]} />
                ))}
              </Pie>
              <Tooltip />
