  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        console.log("Fetched Analytics Data:", result); // Debug log
        setData(result.data);
      } catch (error) {
  ...
  return (
    <div className="h-screen overflow-y-auto bg-gray-100 px-4 py-6">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        <StatCard title="New Visitors" value={data.newUsers ?? 'N/A'} icon="🆕" />
        <StatCard title="Returning Visitors" value={data.returningUsers ?? 'N/A'} icon="🔁" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <motion.div className="bg-white p-4 rounded-2xl shadow" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <h2 className="text-sm font-semibold mb-2 text-gray-700">🗓️ Visitors (7 Days)</h2>
        ...
        <motion.div className="bg-white p-4 rounded-2xl shadow" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={data.sectionCounts || []} layout="vertical">
              ...
              <YAxis dataKey="section" type="category" width={70} />
              <Tooltip />
