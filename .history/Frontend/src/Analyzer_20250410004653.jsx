...
const Analyzer = () => {
  ...
  const normalizeDevices = (devices = []) => {
    ...
    devices.forEach(({ name = "", count = 0 }) => {
      ...
      if (/mobile|iphone|android/.test(lower)) {
        grouped["Mobile"] += count;
      } else if (/tablet|ipad/.test(lower)) {
        grouped["Tablet"] += count;
      } else if (/windows|macbook|linux|desktop|laptop/.test(lower)) {
        grouped["Desktop/Laptop"] += count;
      }
    });
  ...
  return (
    <div className="h-[100vh] overflow-hidden bg-black px-4 py-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
        ...
        {/* Country Visitors - Horizontal Bar Chart with Label */}
